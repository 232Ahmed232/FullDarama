...
export async function POST(request: Request) {
    ...
    const femaleActorId = actId
    console.log(femaleActorId);

    const VotedFemaleActor:any = await Female_actor.findById(femaleActorId)

    if (VotedFemaleActor) {
        VotedFemaleActor.votedBy.push({ user: userId })
        VotedFemaleActor.votes += 1

        await VotedFemaleActor.save()
